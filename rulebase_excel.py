...
class ruleBase(object):
    ...
    ############################################################################################
    def fit(self, df_org, target_attribute_name='target', output_graph = False, save_dir='save_dir', d_set='datanm', rule_rate=0.7):
        df = df_org.copy()
        #df_g= df_g.copy()
...
#                break
            ...
            #############################
            # rule attr list를 위한 코드 #
            #############################
        
            class_number = {}
            for i in elements:    
                class_number[i] = len(df_org[df_org[target_attribute_name] == i])
            ...
            if len(df_org) == len(not_used_idx):
                ...
                for num, graph in enumerate(graph_tree_rule):
                    ...
                    #samples_class_ = leaf_info[3].replace('nsamples/class =', '')
                    ...
                    #print(samples_class_)
                    
                    #print(f'pred_: {pred_}')
                    #print(f'attr_num_: {attr_num_}')
                    #print(f'coverage_: {coverage_}')
                    #print(f'split_criterion: { dic[num]}')
                    #print(f'lift_: { lift_}')
        ...
        return self.rule_model, rule_info
    ...
    def predict(self, org_df, method='average'):
        ...
        for rule_nm in sorted(self.rule_model.keys())[1:]:
            ...
            if method=='priority':
                ...
                #print(sub_set_idx_, len(sub_set_idx_))
