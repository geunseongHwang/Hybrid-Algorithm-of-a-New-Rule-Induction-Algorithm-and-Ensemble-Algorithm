        ...
        for df, rule_dic_ex in zip(self.train_list, self.rule_dic_list):
            ...
            for rule_nm in sorted(rule_dic_ex.keys())[1:]:
                ...
                if rule_nm != 0 and len(base_sub_set_idx) > 0: 
                    ...
                    여기서 예를 들면,
                    처음 rule0이 생성되고
                    rule0에 rule1의 분기기준을 적용하는데 나온 샘플(class0,1)이
                    base로 되는것이고, rule1의 샘플이 adv가 된다.
                    
                    하지만, 코드상 꼬이는 걸 방지하기 위해서,
                    마지막에 base -> adv가 되고
                    adv -> base가 됌.
                    ''' 
                    ...
                    # [설명]
                    # base(rule이 아닌 rule)와 adv(rule)의 클래스별 샘플로
                    # 카이제곱 통계량을 구함
                    # 그렇게 나온 카이제곱 p값이 0.05보다 작으면 "depandent"
                    # 0.05보다 크면 "indepandent"
                    
                    rule_data = {'base':[base_cls0, base_cls1], 'adv':[adv_cls0, adv_cls1]}

                    df_rule_data = pd.DataFrame(rule_data, index=['base','adv'])
                    
                    '''
                    # index가 0이 안나오면 p-값이 나오고 
                    # 0이 나오면 p-값이 안나오는 경우가 생기므로 
                    # 그걸 방지하기 위해 try except 쓰고, indepandent라 가정
                    '''
                    
                    try:
                        chi2, pvalue, dof, expected = scipy.stats.chi2_contingency(df_rule_data)
                    ...
                    sort_ind = ['Sub_set_idx', 'Class_0', 'Class_1', 'Class_predict', \
                                'Total_class_num', 'Homogeneity', \
                                'Coverage', 'Cum_index', 'Rule_determination', \
                                'Rule_attrs','chi2', 'pvalue']
                    ...
                    {sort_col[0] : [len(adv_sub_set_idx), adv_cls0, adv_cls1, adv_pred, \
                                    adv_num_total, adv_homo, \
                                    adv_cover_total, '-', det, 
                                    final_rule_store, chi2,pvalue],
